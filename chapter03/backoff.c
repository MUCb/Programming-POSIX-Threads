...
void *lock_backward (void *arg)
{
	...
	for (iterate = 0; iterate < ITERATIONS; iterate++)
	{
		 ...
		 * Report that we got 'em, and unlock to try again.
		 */
		printf(" lock backward got all locks, %d bacloffs\n\n", backoffs);
		pthread_mutex_unlock (&mutex[0]);
		pthread_mutex_unlock (&mutex[1]);
		pthread_mutex_unlock (&mutex[2]);
		sched_yield ();
...
int main(int argc, char *argv[])
{
	...
	// #ifndef sun
		/*
		 * On Solaris 2.5, thread are not timesliced. To ensure
		 * that our thread can run concurrently, we need to 
	 ...
	 * run "at speed." On some system, especially uniprocessors,
