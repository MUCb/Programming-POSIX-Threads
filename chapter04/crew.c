...
void *worker_routine(void *arg){
     ...
     */
     ...
     while (1) {
         ...
         if (S_ISLNK (filestat.st_mode))
             ...
             */
             ...
             while (1) {
                ...
                crew->work_count++;
                DPRINTF (( "Crew %d: add %#lx, last %#lx, %d\n", mine->index, 
                    new_work, crew->first, crew->last, crew->work_count));
         ...
         * calculation) if the new crew is now idle.
         *
         * It's important that the count be decremanted AFTER
         * processing the current work item. That ensures the 
         * count won't go to 0 untill we're really done.
         */
         status = pthread_mutex_lock (&crew->mutex);
         if (status != 0)
            err_abort (status, "Lock crew mutex");
...
int crew_create (crew_t *crew, int crew_size)
{
     ...
     */
     for (crew_index = 0; crew_index < CREW_SIZE; crew_index++) {
        ...
        status = pthread_create (&crew->crew[crew_index].thread, NULL, 
...
/*
 * Pass a file path to a work crew previosly created using crew_create
 */
