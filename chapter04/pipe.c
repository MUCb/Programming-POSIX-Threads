...
/*
 * Internal structure describing a "stage" in the pipeline. One for each thread,
 * plus a "result stage" where the final thread can stash the value.
 */
...
/*
 * The thread start routine for pipe stage threads. Each will wait for a data
 * item passed from the caller or the previous stage, modify the data and pass
 * it along to the next (or final) stage.
 */
void *pipe_stage (void *arg)
{
    ...
    /*
     * Notice that the routine never unlocks the stage->mutex. The call to
     * pthread_cond_wait implicitly unlocks the mutex while the thread is
     * waiting, allowing other threads to make progress. Because the loop
     * never terminates, this function has no need to unlock the mutex
     * explicitly.
     */
}

/*
 * External interface to create a pipeline. All the data is initialized and
 * the threads created. They'll wait for data.
 */
int pipe_create (pipe_t *pipe, int stages)
{
    ...
    *link = (stage_t*)NULL;     /* Terminate list */
    ...
    /*
     * Create the threads for the pipe stages only after all the data is 
     * initialized (including all links). Note that the last stage doesn't get 
     * a thread, it's just a receptacle for the final pipeline value.
     *
     * At this piont, proper cleanup on an error would take up more space than
     * worthwhile in a "simple example," so instead of cancelling and detaching 
     * all the threads already created, plus the synchronization object and
     * memory cleanup done for earlier errors, it will simply abort.
     */
...
/*
 * External inreface to start a pipeline by passing data to the first stage.
 * The routine returns while the pipeline processes in parallel. Call the 
 * pipe_result return to collect the final stage values (note that the pipe
...
/*
 * Collect the result of the pipeline. Wait for a result if the pipeline hasn't
 * produced one.
 */
