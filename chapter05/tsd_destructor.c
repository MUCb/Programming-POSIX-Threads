 ...
 * for the thread-specific dataq key. It keeps track of how many
...
/*
 * Thread start routine to use thread-specidic data.
 */
...
int main (int argc, char *argv[])
{
     ...
     * the member of threads that will use it (two thread_routine
     * threads plus main). This must be done before creating
     * the threads! Otherwise, if one thread runs the key's 
     * destructor befor any other thread uses the key, it will
     * be deleted.
     *
