...
int main (int argc, char *argv[])
{
     ...
     * initila thread. Because all threads inherit the signal mask
     * from their creator, all the process will have
     * SIGINT masked unless one explicitly umask it. The
     * semantics of sigwait requires that all threads (including
     * the thread calling sigwait) have the signal masked, for
     * reliable operation. Otherwise, a signal that arives
    ...
    /*
     * Wait for the sigwait thread to recive SIGINT and signal
